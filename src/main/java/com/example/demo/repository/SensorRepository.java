...
public interface SensorRepository extends JpaRepository<SensorEntity, Long> {
    @Query("SELECT s FROM SensorEntity s WHERE "
            + "(:temperature IS NULL OR s.temperature = :temperature) AND "
            + "(:humidity IS NULL OR s.humidity = :humidity) AND "
            + "(:light IS NULL OR s.light = :light) AND "
            + "(:timestamp IS NULL OR s.timestamp = :timestamp)")
    List<SensorEntity> findByParams(
